...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--fundo2)]/95 backdrop-blur-md border-b border-[var(--bordadg)] transition-colors duration-200 py-1.5 sm:py-2">
        ...
        <div className="flex items-center justify-between gap-2 min-h-[38px]">
          ...
          <nav className="flex items-center gap-1.5 sm:gap-2">
            <button
              ...
              className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                activeTab === 'arvore'
      ...
      <div className="md:hidden border-t border-[var(--bordadg)] mt-1.5 pt-1.5 px-3 flex items-center justify-around gap-2">
        <button
          type="button"
          onClick={() => setActiveTab('arvore')}
          className={`flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold py-1 px-2 rounded-lg transition-colors cursor-pointer ${
